tradliteApp.component("chart", {
    templateUrl: "/components/chart/chart.html",
    controller: function ($scope, $http, $q, $window) {
        var width;
        var dim;
        var parseDate;
        var zoom;
        ...
        $scope.load_chart = function () {
            var request = { ticker: $scope.ticker, fromDate: $scope.fromDate, toDate: $scope.toDate, importer: $scope.importer };
            var promise1 = $http({ url: "/api/candles", method: "GET", params: request });
            var promise2 = $http({ url: "/api/buysignalindicies", method: "GET", params: request });
            var promise3 = $http({ url: "/api/sellsignalindicies", method: "GET", params: request });
            $q.all([promise1, promise2, promise3]).then(function (responses) {
                console.log(responses);
                candles = responses[0].data;
                buyRules = responses[1].data;
                sellRules = responses[2].data;
                drawChart();
            });
        }
        
        function init() {
            $scope.importers = ["Yahoo", "Google", "Quandl", "Stooq", "Csv"];
